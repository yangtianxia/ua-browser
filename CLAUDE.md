...
`src/detectors/device.ts` accepts a `strategy` param: `'ua-first'` skips hardware signals, `'hardware-first'` prioritises them, `'strict'` returns `'unknown'` on disagreement.
